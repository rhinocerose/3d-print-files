...
spaceBetween = 14; // [1:0.1:100]
teethHeight = 15; // [1:0.1:20]

// Count of cable mounts
size = 3; // [1:20]

/* [hidden] */
width = 28;
height = teethHeight * 1.8;
thickness = 5;
    
function concatArray(L1, L2) = [for(L=[L1, L2], a=L) a];
function flatten(l) = [for (a = l) for (b = a) b];
    
module fullComb() {
    function getPoints(i) = [
        [i * spaceBetween, thickness, i == 0 ? 0 : thickness * 0.4],
        [((i+1) * spaceBetween) - thickness, thickness, thickness * 0.4],
        [((i+1) * spaceBetween) - thickness, teethHeight, thickness],
        [((i+1) * spaceBetween), teethHeight, thickness],
    ];
    
    module comb() {
        combPoints = flatten([for (i = [0: size-1]) getPoints(i)]);
        leftCombPoints = [[0, 0, 0]];
        leftCombEnd = [[size * spaceBetween, 0, thickness]];
        leftComb = concatArray(leftCombPoints, combPoints);
        points = concatArray(leftComb, leftCombEnd);
...
module rack() {
    leftRackWidth = 28;
    rackThickness = thickness / 2;
    rackBaseThickness = thickness * 2;
    
    points = [
        [-rackThickness, 0, 0],
        [-rackThickness, height, thickness],
        [-leftRackWidth, height, 3],
        [-leftRackWidth, height + rackBaseThickness, 3],
        [leftRackWidth, height + rackBaseThickness, 3],
        [leftRackWidth, height, 3],
        [rackThickness, height, thickness],
        [rackThickness, 0, 0],
...
module multiConnectSlider() {
    tollerance = 0.4;
    bottomWidth = 20 + tollerance;
    topWidth = 15 + tollerance;
    height = 4;
    radius = 8;
    spacingForConnect = 1;
    
    module base() {
        linear_extrude(height = spacingForConnect) {
            circle(d = bottomWidth);
        }
        translate([0, 0, 1]) {
            cylinder(d1 = bottomWidth, d2 = topWidth, h = height - spacingForConnect);
        }
        
        levelDiff = (bottomWidth - topWidth) / 2;
        points = [
            [0, 0, 0],
            [bottomWidth, 0, 0],
            [bottomWidth, spacingForConnect, 0],
            [topWidth + levelDiff, height, 0],
            [levelDiff, height, 0],
            [0, spacingForConnect, 0]
        ];
        translate([0, -(bottomWidth / 2), 0]) {
            rotate([90, 0, 90]) {
...
module cableHook() {
    ...
    translate([14, height + 6, width - 14]) {
        rotate([0, 90, 90]) {
            multiConnectSlider();
        }
    }
